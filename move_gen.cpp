...
uint64_t getBishopMoves(uint64_t pos, Board &board, bool isWhite) {
  ...
  for (int i = 1; i < 8; i++) {
    ...
    attack |= s;
    if (board.allPieces & s) { if (s & sameColorPieces) attack ^= s; break; }
  }
  for (int i = 1; i < 8; i++) {
...
uint64_t getQueenMove(uint64_t pos, Board &board, bool isWhite) {
  return getRookMoves(pos, board, isWhite) | getBishopMoves(pos, board, isWhite);
}
...
uint64_t getPawnMoves(uint64_t pos, Board &board, bool isWhite) {
  ...
  if (isWhite) {
    ...
    if (single && !(single & board.allPieces)) {
      moves |= single;
      if ((pos & 0xFF00) && !(single << 8 & board.allPieces)) moves |= (single << 8);
    }
    ...
    if (board.enPassantSq != -1) {
       ...
       if ((leftCap | rightCap) & epBit) moves |= epBit;
    }
  } else {
    uint64_t single = pos >> 8;
    if (single && !(single & board.allPieces)) {
      moves |= single;
      if ((pos & 0x00FF000000000000ULL) && !(single >> 8 & board.allPieces)) moves |= (single >> 8);
    }
...
bool isKingInCheck(Board &board, bool isWhite) {
  ...
  if (isWhite) {
    if (((kingPos << 7) & ~board.hfile & oppPawns) || ((kingPos << 9) & ~board.aFile & oppPawns)) return true;
  } else {
    if (((kingPos >> 7) & ~board.aFile & oppPawns) || ((kingPos >> 9) & ~board.hfile & oppPawns)) return true;
  }
  ...
  while (oppKnights) {
    ...
    if (getKnightMoves(pos, board, opp) & kingPos) return true;
  ...
  auto checkSlider = [&](uint64_t pieces, auto func) {
    while (pieces) {
      ...
      if (func(pos, board, opp) & kingPos) return true;
      pieces &= pieces - 1;
    }
    return false;
  };
  if (checkSlider(isWhite ? board.blackBishops : board.whiteBishops, getBishopMoves)) return true;
  if (checkSlider(isWhite ? board.blackRooks : board.whiteRooks, getRookMoves)) return true;
  if (checkSlider(isWhite ? board.blackQueen : board.whiteQueen, getQueenMove)) return true;

  // King
  uint64_t oppKing = isWhite ? board.blackKing : board.whiteKing;
  if (oppKing && (getKingMoves(oppKing, board, opp) & kingPos)) return true;
...
vector<Move> generateAllMoves(Board &board, bool isWhite) {
  vector<Move> moves;
  auto addMove = [&](int from, int to, PieceType p, PieceType cap, bool ep = false, bool cast = false, PieceType prom = NONE) {
    Move m = {from + 1, to + 1, p, cap, prom, cast, ep};
    Board temp = board;
    makeMove(temp, m, isWhite);
    if (!isKingInCheck(temp, isWhite)) moves.push_back(m);
  };
  ...
  for (int i = 0; i < 64; i++) {
    ...
    if (bit & (isWhite ? board.whitePawns : board.blackPawns)) {
      type = PAWN; targets = getPawnMoves(bit, board, isWhite);
    } else if (bit & (isWhite ? board.whiteKnights : board.blackKnights)) {
      type = KNIGHT; targets = getKnightMoves(bit, board, isWhite);
    } else if (bit & (isWhite ? board.whiteBishops : board.blackBishops)) {
      type = BISHOP; targets = getBishopMoves(bit, board, isWhite);
    } else if (bit & (isWhite ? board.whiteRooks : board.blackRooks)) {
      type = ROOK; targets = getRookMoves(bit, board, isWhite);
    } else if (bit & (isWhite ? board.whiteQueen : board.blackQueen)) {
      type = QUEEN; targets = getQueenMove(bit, board, isWhite);
    } else if (bit & (isWhite ? board.whiteKing : board.blackKing)) {
      type = KING; targets = getKingMoves(bit, board, isWhite);

      // Castling
      if (!isKingInCheck(board, isWhite)) {
        if (isWhite) {
          if ((board.castlingRights & 1) && !(board.allPieces & 0x60) && !isKingInCheck(board, true)) { // Kingside
             // Need to check intermediate square
             Board t = board; Move m = {5, 6, KING, NONE}; makeMove(t, m, true);
             if (!isKingInCheck(t, true)) addMove(4, 6, KING, NONE, false, true);
          }
          if ((board.castlingRights & 2) && !(board.allPieces & 0x0E)) { // Queenside
             Board t = board; Move m = {5, 4, KING, NONE}; makeMove(t, m, true);
             if (!isKingInCheck(t, true)) addMove(4, 2, KING, NONE, false, true);
          }
        } else {
          if ((board.castlingRights & 4) && !(board.allPieces & (0x60ULL << 56))) {
             Board t = board; Move m = {61, 62, KING, NONE}; makeMove(t, m, false);
             if (!isKingInCheck(t, false)) addMove(60, 62, KING, NONE, false, true);
          }
          if ((board.castlingRights & 8) && !(board.allPieces & (0x0EULL << 56))) {
             Board t = board; Move m = {61, 60, KING, NONE}; makeMove(t, m, false);
             if (!isKingInCheck(t, false)) addMove(60, 58, KING, NONE, false, true);
          }
        }
      }
    }
    ...
    while (targets) {
      ...
      if (toBit & (isWhite ? board.blackPawns : board.whitePawns)) cap = PAWN;
      else if (toBit & (isWhite ? board.blackKnights : board.whiteKnights)) cap = KNIGHT;
      else if (toBit & (isWhite ? board.blackBishops : board.whiteBishops)) cap = BISHOP;
      else if (toBit & (isWhite ? board.blackRooks : board.whiteRooks)) cap = ROOK;
      else if (toBit & (isWhite ? board.blackQueen : board.whiteQueen)) cap = QUEEN;
      else if (toBit & (isWhite ? board.blackKing : board.whiteKing)) cap = KING;
